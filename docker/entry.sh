...
if [[ ! -z "$LDAP_SERVER" ]]; then
        sed -i "s|LDAP_SERVER|$LDAP_SERVER|"  /etc/proftpd/ldap.conf
fi

if [[ ! -z "$LDAP_GROUP" ]]; then
        sed -i "s|LDAP_GROUP|$LDAP_GROUP|"  /etc/proftpd/ldap.conf
fi

if [[ ! -z "$LDAP_GROUP" ]]; then
        sed -i "s|LDAP_BASEDN|$LDAP_BASEDN|"  /etc/proftpd/ldap.conf
fi

if [[ ! -z "$LDAP_USER" ]]; then
        sed -i "s|LDAP_USER|$LDAP_USER|"  /etc/proftpd/ldap.conf
fi

if [[ ! -z "$LDAP_PWD" ]]; then
        sed -i "s|LDAP_PWD|$LDAP_PWD|"  /etc/proftpd/ldap.conf
fi
